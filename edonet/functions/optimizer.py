    ...
    def __init__(self, model, beta1=0.9, beta2=0.999, epsilon=1e-8):
        ...
        for layer in model.layers:
            if layer.has_weights:
                ...
                self.moments1_w[layer.index] = np.zeros(layer.weights.shape)
                self.moments2_w[layer.index] = np.zeros(layer.weights.shape)
                self.moments1_b[layer.index] = np.zeros(layer.bias.shape)
                self.moments2_b[layer.index] = np.zeros(layer.bias.shape)
